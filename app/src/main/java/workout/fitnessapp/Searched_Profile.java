...
public class Searched_Profile extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.searched_profile);
        ...
        ProfileInformation2(Search_Adapter.search_uid);
        ...
        Objects.requireNonNull(getSupportActionBar()).setTitle(Search_Adapter.Search_name);
...
//                TextView Vtext=view2.findViewById(R.id.VImgtext);
                viewimg.setOnClickListener(view5->{
                    ...
                    intent.putExtra("dp",Search_Adapter.search_uid);
...
//                TextView Vtext=view2.findViewById(R.id.VImgtext);
                viewimg.setOnClickListener(view5->
                {
                    ...
                    intent.putExtra("dc",Search_Adapter.search_uid);
                    startActivity(intent);
                    alertDialog.cancel();
                });
            }
        ...
        Trainers.setOnClickListener(view ->
        {
            if (trai!=1)
            {
                Trainners trainners=new Trainners(Search_Adapter.search_uid);
                reference.child(Menu_Home_page.CurrentLoginUserId).child("trainner").push().setValue(trainners).addOnSuccessListener(new OnSuccessListener<Void>() {
                    ...
                    public void onSuccess(Void unused) {
                        ...
                        trai=1;
                        Toast.makeText(Searched_Profile.this, "Trainner added", Toast.LENGTH_SHORT).show();
                        Trainner_Followers trainner_followers=new Trainner_Followers(Menu_Home_page.CurrentLoginUserId);
                        reference.child(Search_Adapter.search_uid).child("trainnerFollowers").push().setValue(trainner_followers);
                    }
            ...
            else
            {
                reference.child(Menu_Home_page.CurrentLoginUserId).child("trainner").child(isTrainnerList.get(0).getTrainnerKey()).child("trainner").removeValue().addOnSuccessListener(new OnSuccessListener<Void>() {
                    ...
                    public void onSuccess(Void unused) {
                        ...
                        trai=0;
                        Toast.makeText(Searched_Profile.this, "Trainner removed", Toast.LENGTH_SHORT).show();
                        reference.child(Search_Adapter.search_uid).child("trainnerFollowers").child(isTrainnerfollowList.get(0).getTrainnerFollowersKey()).child("trainnerFollowers").removeValue();
                    }
                }).addOnFailureListener(fail->
                {
                    Toast.makeText(this, fail.getMessage(), Toast.LENGTH_SHORT).show();
        ...
        startfollow.setOnClickListener(view ->
        {
            if (foll!=1)
            {
                Following following=new Following(Search_Adapter.search_uid);
                reference.child(Menu_Home_page.CurrentLoginUserId).child("following").push().setValue(following).addOnSuccessListener(new OnSuccessListener<Void>() {
                    ...
                    public void onSuccess(Void unused) {
                        ...
                        foll=1;
                        Toast.makeText(Searched_Profile.this, "Following", Toast.LENGTH_SHORT).show();

                        ////////////////////////////////////////////////////////////////
                        Followers followers=new Followers(Menu_Home_page.CurrentLoginUserId);
                        reference.child(Search_Adapter.search_uid).child("followers").push().setValue(followers);
                    }
                }).addOnFailureListener(fail ->
                {
                    Toast.makeText(this, fail.getMessage(), Toast.LENGTH_SHORT).show();
            ...
            else
            {
                reference.child(Menu_Home_page.CurrentLoginUserId).child("following").child(isFollowList.get(0).getFollowingKey()).child("following").removeValue().addOnSuccessListener(new OnSuccessListener<Void>() {
                    ...
                    public void onSuccess(Void unused) {
                        ...
                        foll=0;
//                        setFollowBackground();
                        Toast.makeText(Searched_Profile.this, "Unfollowed", Toast.LENGTH_SHORT).show();

                        reference.child(Search_Adapter.search_uid).child("followers").child(isFollowerList.get(0).getFollowerKey()).child("followers").removeValue();
                    }
                }).addOnFailureListener(fail->
                {
                    Toast.makeText(this, fail.getMessage(), Toast.LENGTH_SHORT).show();
        ...
        reference.child(Menu_Home_page.CurrentLoginUserId).child("following").addValueEventListener(new ValueEventListener() {
            ...
            public void onDataChange(@NonNull DataSnapshot snapshot)
            {
                for (DataSnapshot snapshot1:snapshot.getChildren())
                {
                    ...
                    if(Search_Adapter.search_uid.equals(following.getFollowing()))
                    {
                        isFollowList.clear();
    ...
    {
        reference.child(Search_Adapter.search_uid).child("followers").addValueEventListener(new ValueEventListener() {
            @SuppressLint("SetTextI18n")
            @Override
            public void onDataChange(@NonNull DataSnapshot snapshot)
            {
    ...
    {
        reference.child(Search_Adapter.search_uid).child("following").addValueEventListener(new ValueEventListener() {
            @SuppressLint("SetTextI18n")
            @Override
            public void onDataChange(@NonNull DataSnapshot snapshot)
            {
        ...
        reference.child(Menu_Home_page.CurrentLoginUserId).child("trainner").addValueEventListener(new ValueEventListener() {
            ...
            public void onDataChange(@NonNull DataSnapshot snapshot)
            {
                for (DataSnapshot snapshot1:snapshot.getChildren())
                {
                    ...
                    if(Search_Adapter.search_uid.equals(trainners.getTrainner()))
                    {
                        Trainers.setText("Trainner added");
    ...
    {
        reference.child(Search_Adapter.search_uid).child("trainnerFollowers").addValueEventListener(new ValueEventListener() {
            @SuppressLint("SetTextI18n")
            @Override
            public void onDataChange(@NonNull DataSnapshot snapshot)
            {
            ...
            public void onComplete(@NonNull Task<DataSnapshot> task) {
                if (task.isSuccessful())
                {
                    ...
                    if (task.getResult().exists())
                    {
                        ...
                        {
                            Glide.with(Searched_Profile.this)
                                    .load(Profilepic)
                                    .centerCrop()
...
//                            textView10.setVisibility(View.GONE);
                ...
                else
                {
                    progressBar.setVisibility(View.INVISIBLE);
                    scrollView2.setVisibility(View.VISIBLE);
                    Toast.makeText(Searched_Profile.this, "Loading Failed", Toast.LENGTH_SHORT).show();
                }
            }
        }).addOnFailureListener(fail->
        {
            progressBar.setVisibility(View.INVISIBLE);
            scrollView2.setVisibility(View.VISIBLE);
            Toast.makeText(Searched_Profile.this, "Loading Failed.... "+fail.getMessage(), Toast.LENGTH_SHORT).show();
        });
    }
